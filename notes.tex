...
\begin{frame}{Projective (von Neumann) Measurement}
	...
	\customcite[1]{wiseman_quantum_2010}
\end{frame}
...
\begin{frame}{Zeno Effect}
	\begin{columns}
		...
		\end{itemize}
		\customcite{kominis_quantum_2009}
...
\begin{frame}{Practical Examples of strong measurements}
	\begin{columns}
		...
		\only<5>{
			...
			But breaks on:
			\begin{itemize}
				\item simultaneous measurement of conjugate variables
...
\begin{frame}{Weak Measurement}
	...
	\end{center}
	\only<-1>{
		Measured system $S$ and Measurement Device $S$ combined to:
		$$H = H_S\otimes 1 + 1\otimes H_M + g(t) \; C_S \otimes P_M$$
		
		Then a strong measurement $X$ is performed on the $M$\\
		this yields $\langle C\rangle + \xi$ and partial collapse in $S$
	}
	...
	\customcite[]{tamir_introduction_2013}
\end{frame}
...
\begin{frame}{Example: Weak measurment}
	\begin{columns}
		\column{.5\textwidth}
		\begin{center}
